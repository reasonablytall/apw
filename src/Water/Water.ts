class Water {
    ...
    impulse(x: number, strength: number = 50, width: number = 1) {
        for (let i = x - Math.floor(width / 2); i < x + Math.ceil(width / 2); i++) {
            if (inArr(i, this.v)) this.v[i] += strength;
        }
    }
    ...
    tankCoords(px: number, py: number) {
        let dims = this.tankDimensions();
    ...
    tankDimensions(): [number, number, number, number] {
        let charwidth = parseInt(window.getComputedStyle(this.tank).lineHeight as string, 10) * 0.835;
        let tankwidth = this.tank.clientWidth;
        let tankcols = Math.ceil(tankwidth / charwidth);
        let lineheight = parseInt(window.getComputedStyle(this.tank).lineHeight as string, 10);
...
function inArr(i: number, arr: any[]) {
...
function renderAscii(pixels: AsciiPixel[]): string {
...
function getBits(flags: number[][], x: number, y: number): number {
...
function getChar(flags: number[][], x: number, y: number): string {
...
var w: Water;
window.onpageshow = () => {
    let b = document.getElementById('water') as HTMLElement;
    w = new Water(b);
